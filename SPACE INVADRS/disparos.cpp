...
#include <allegro.h>
bool colicion(int x1,int y1,int w1,int h1,
...
bool elimina_bala_objeto(struct NAVE& N, struct NAVE& E, struct Balas B[]){
          if ( N.n_disp > 0 && N.n_disp < N.max_disp){
            for ( int cont = 1; cont <= N.n_disp; cont++){
                if(colicion(E.x , E.y , E.ancho_p, E.alto_p,
...
void elimiar_bala_escudo(struct NAVE &N, struct escudo ES[],struct Balas B[]){
    if ( N.n_disp > 0 && N.n_disp < N.max_disp){
            for ( int cont = 1; cont <= N.n_disp; cont++){
                for(int i=0;i<20;i++){
                    if(colicion(ES[i].x ,ES[i].y ,20 ,16 ,B[cont].x ,B[cont].y,
                                N.ancho_b, N.alto_b) && ES[i].dan < 3){
